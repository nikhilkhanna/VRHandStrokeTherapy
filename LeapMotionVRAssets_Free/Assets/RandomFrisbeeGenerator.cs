...
public class RandomFrisbeeGenerator : MonoBehaviour {
	public float radius;
	...
	void Update () {
		if (Time.time-preTime > spawnTime)
		{
			...
			Vector3 position = new Vector3(
				x + 2 * dev * Mathf.Cos(theta),
				y + dev * Mathf.Sin(theta),
				z
				);
...
//			sphere.transform.rotation = Quaternion.Euler(Random.Range(0.0f, 360.0f), Random.Range(0.0f, 360.0f), Random.Range(0.0f, 360.0f));
			sphere.transform.localScale = new Vector3(radius, thickness, radius);
			sphere.transform.localPosition = position;
			sphere.AddComponent<Rigidbody>();
			sphere.rigidbody.useGravity = false;

			sphere.rigidbody.velocity = new Vector3(0, 0, (float) Mathf.Max ((float) (-speed-.1*Time.time), -1.2f));
			spawnTime = (float) Mathf.Max ((float) (.95 * spawnTime), 1.0f);
			sphere.renderer.material.color = list_of_colors[Random.Range(0,list_of_colors.Length - 1)];
			sphere.AddComponent ("Frisbee");
